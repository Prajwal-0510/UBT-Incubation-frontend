  ...
  if (isAdmin) return (
    <div style={{ paddingTop:68, minHeight:'100vh', background:'#f8f7f4' }}>
      <div style={{ maxWidth:720, margin:'0 auto', padding:'80px 24px' }}>
        <div className="animate-scale-in" style={{ background:'#fff', borderRadius:20, padding:'48px', boxShadow:'0 8px 40px rgba(5,13,26,0.1)', border:'1px solid #e5e4e0', textAlign:'center' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:14, marginBottom:36 }}>
            {[
              ...
              { label:'👨‍🎓 Manage Alumni',    page:'alumni' },
              { label:'📢 Manage Updates',   page:'manage-updates' },
              { label:'📋 View Inquiries',   page:'inquiries' },
            ].map((a,i) => (
              <button key={i} onClick={() => onNavigate(a.page)} style={{ background:'#050d1a', color:'#e8b84b', border:'none', padding:'14px 20px', borderRadius:10, cursor:'pointer', fontWeight:600, fontSize:13.5, fontFamily:'Outfit, sans-serif', transition:'all 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.background='#0a1628'}
                onMouseLeave={e => e.currentTarget.style.background='#050d1a'}
              >{a.label}</button>
            ))}
          </div>
          <button onClick={logout} style={{ background:'#fee2e2', color:'#dc2626', border:'none', padding:'10px 24px', borderRadius:8, cursor:'pointer', fontWeight:600, fontSize:13, fontFamily:'Outfit, sans-serif' }}>🚪 Logout</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ paddingTop:68, minHeight:'100vh', background:'linear-gradient(135deg,#050d1a,#0a1628 50%,#0f2044)', display:'flex', alignItems:'center' }}>
      <div style={{ maxWidth:440, margin:'0 auto', padding:'40px 24px', width:'100%' }}>
        <div className="animate-scale-in" style={{ background:'rgba(255,255,255,0.05)', backdropFilter:'blur(20px)', border:'1px solid rgba(232,184,75,0.2)', borderRadius:20, padding:'48px 40px' }}>
          <div style={{ textAlign:'center', marginBottom:36 }}>
            <div style={{ width:64, height:64, background:'linear-gradient(135deg,#e8b84b,#fcd34d)', borderRadius:16, margin:'0 auto 18px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:30, boxShadow:'0 8px 24px rgba(232,184,75,0.4)' }}>🔐</div>
